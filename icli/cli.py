...
class IBKRCmdlineApp:
    ...
    predicate_engine: Any = field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        self.predicate_engine = PredicateEngine(
            ...
            positional_quote_repopulate=self.positionalQuoteRepopulate,
            add_quote_from_contract=self.addQuoteFromContract,
            name_for_contract=self.nameForContract,
            algobinder_read=lambda field: self.algobinder.read(field) if self.algobinder else None,
            ensure_algobinder=lambda: self.algobindertask or self.algobinderStart(),
            portfolio_position=lambda conId: self.ib.wrapper.portfolio[self.accountId][conId],
        )
    ...
    async def predicateSetup(self, prepredicate: ifthen.CheckableRuntime):
        ...
        await self.predicate_engine.setup(
            prepredicate,
            task_create_fn=self.task_create,
            extension_fns={
                "verticalput": self.ifthenExtensionVerticalSpreadPut,
                "verticalcall": self.ifthenExtensionVerticalSpreadCall,
                "position": self.ifthenQuantityForContract,
                "abs": self.ifthenAbs,
            },
        )
    ...
    def dataExtractorForTicker(self, iticker: ITicker, field: str, timeframe: int):
        ...
        return self.predicate_engine.data_extractor_for_ticker(iticker, field, timeframe)
